...
from a rational point on a curve C admitting a degree 2 map X --> C.
It is also possible to add to the IsLonely function
...
/* INPUT: 
- a model for projective curve X/\Q,
- a set L of degree 4 divisors on X,
- a set divs of degree 0 divisors that generate a finite index subgroup G of J_X(Q), 
- an integer I such that I*J_X(Q) subset G,
- an abstract abelian group A isomorphic to G such that A.i corresponds to divs[i], 
- a degree 4 divisor bp on X, to be used for the map X^{(4)} --> J_X,
- an automorphism w5 on X such that we do Chabauty relative to X --> X/w_5,
- a subgroup B0 of A embedded via iA0: B0 --> A and a set W0 of B-coset representatives in A.
The triple (B0,iA0,W0) is needed to start the sieve. 
Without prior assumptions, these should be B0=A, iA0=Id and W0={0}. */

//The below function verifies the conditions of the partially relative Chabauty theorem
...
for x in imW do //For each z such that I*z = x, we check whether z can come from a rational point. If yes for some z, we keep x. 
    z:=x@@mI;
    if &or[Dimension(phi(z+k)+bpp) gt 0 and (not z+k in redL2 
           or p in [11,13] or not IsLonely(L[Index(redL2,z+k)],p,X,w5)) : k in K] 
    then Append(~jposP,x);
    end if;
end for;
...
if W eq [] then "The sieve has terminated successfully."; 
    return true;
end if;
